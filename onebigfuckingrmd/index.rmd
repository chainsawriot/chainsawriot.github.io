...
今晚在 [SHOW ME THE CODE](https://github.com/code4hk/showmethecode/issues/11) meetup 講完 Rcpp 。所以暫停寫在 `pipe` 發現的東西（應該還有一件事想寫），先將今晚的分享轉成一份 notes ，是以寫給 R Programmer 看的。
...
以下是我將 R 的 `cosine` 和 `census` 部份重寫成 C++ 的樣子。
...
結講是，對 R programmer 來說 C++ 仍是值得一學。要達到以上的加速效果，並不需要使用到很高深的功能（如 p
搜尋結果 Pointer Arithmetic ，但其實 Rcpp 背後已替你將 R 輸入的東西變成 pass by reference ，減少複製 value ）。我正在研究如何使用 Rcpp 令 readODS 的 XML 處理變得更快。
