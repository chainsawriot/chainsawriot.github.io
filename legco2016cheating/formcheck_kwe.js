...
$(document).ready(function(){
    $("p").html(print_recount(recount(kwe_metadb, kwe_db, kwe_db().select('code'))))
    var options = kwe_db().map(function(item) { return({'name': 'station', 'caption':item["cname"], 'value': item['code'], 'type': 'checkbox', 'checked': 'checked'}) })
    var foptions = []
...
//	var res = kwe_db({'code':selected})
//	var stations = res.map(function (item) { return(item['cname']) })
	    $("p").html(print_recount(recount(kwe_metadb, kwe_db, selected)))
    })
})
