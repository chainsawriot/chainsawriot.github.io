...
$(document).ready(function(){
    $("p").html(print_recount(recount(hk_metadb, hk_db, hk_db().select('code'))))
    var options = hk_db().map(function(item) { return({'name': 'station', 'caption':item["cname"], 'value': item['code'], 'type': 'checkbox', 'checked': 'checked'}) })
    var foptions = []
...
//	var res = hk_db({'code':selected})
//	var stations = res.map(function (item) { return(item['cname']) })
	    $("p").html(print_recount(recount(hk_metadb, hk_db, selected)))
    })
})
